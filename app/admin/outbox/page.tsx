...
import { getOutboxTail } from "@/lib/mailer";
...
function safeParse(line: string): OutboxRow | null {
  try {
    return JSON.parse(line) as OutboxRow;
  } catch {
    return null;
  }
}
...
export default async function OutboxAdminPage({ searchParams }: { searchParams: { token?: string } }) {
  ...
  const lines = await getOutboxTail(250);
  const rows = lines.map(safeParse).filter(Boolean) as OutboxRow[];

  rows.sort((a, b) => {
    const ta = new Date(a.created_at || a.payload?.created_at || 0).getTime();
    const tb = new Date(b.created_at || b.payload?.created_at || 0).getTime();
    return tb - ta;
  });
